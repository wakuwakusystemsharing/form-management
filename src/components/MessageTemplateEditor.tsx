...
export interface MessageTemplateEditorProps {
  /** ブロックの見出し（例: リマインダー文面のカスタマイズ） */
  title: string;
  /** 空欄時に使われるデフォルト文言（placeholder 表示用） */
  defaults: { headerTitle: string; bodyText: string; footerText: string };
  /** 本文の入力例（placeholder の 2 行目以降） */
  bodyExample: string;
  /** プレビューに使うサンプル予約 */
  sampleContext: ReminderContext;
  /** テンプレート + サンプルから表示内容を解決する（リマインダー / フォローで異なるデフォルト） */
  resolve: (template: ReminderTemplate | null | undefined, ctx: ReminderContext) => ResolvedMessageContent;
  /** プレビュー下の補足（詳細ブロックの意味など） */
  previewNote?: string;
...
/**
 * LINE Flex メッセージ文面（ヘッダー・色・本文・詳細ブロック・末尾）の編集 + プレビュー。
 * 予約リマインダーとフォローメッセージで共用（デフォルト文言・サンプルだけ props で切り替える）。
 */
export default function MessageTemplateEditor({
  title, defaults, bodyExample, sampleContext, resolve, previewNote, value, onChange,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          <div className="space-y-1">
            ...
            <Input value={t.header_title || ''} onChange={(e) => patch({ header_title: e.target.value })} placeholder={defaults.headerTitle} />
          </div>
          ...
          <div className="space-y-1">
            ...
            <Input value={t.footer_text || ''} onChange={(e) => patch({ footer_text: e.target.value })} placeholder={defaults.footerText} disabled={t.show_footer === false} />
            <label className="flex items-center gap-2 text-sm cursor-pointer">
        ...
        <div className="space-y-1">
          ...
          <p className="text-[11px] text-muted-foreground">
            実際の LINE では画面幅に合わせて表示されます。サンプル: LINE名「はなこ」/ お名前「山田 花子」
