...
```env
# 環境切り替え: local / staging / production / development
NEXT_PUBLIC_APP_ENV=local
# local: JSON ファイル永続化、認証スキップ
# staging: 既存の Supabase staging プロジェクトを使用
# production: 新規の独立した Supabase production プロジェクトを使用
# development: dev ブランチ向け Vercel プレビュー環境（staging の dev ブランチ）

# Supabase 設定（ローカルは不要、staging/production/development では必須）
...
- Supabase Storage は Supabase プロジェクトレベルで分離されているため、パスプレフィックスは不要（`reservations/` と `surveys/` のみ）
...
**注意**:
- フォームHTMLは以下のパス構造で保存されます（プロジェクトレベルで staging/production が分離されているため、環境プレフィックスは不要）
  - 予約フォーム: `reservations/{storeId}/{formId}/index.html`
  - アンケートフォーム: `surveys/{storeId}/{formId}/index.html`
...
| 項目 | local | development | staging | production |
|------|-------|-------------|---------|------------|
| `NEXT_PUBLIC_APP_ENV` | `local` | `development` | `staging` | `production` |
| データ永続化 | JSON ファイル | Supabase (staging の dev ブランチ) | Supabase (既存プロジェクト) | Supabase (新規プロジェクト、Pro プラン) |
| Storage デプロイ | `/public/static-forms/` | Supabase Storage (`reservations/`, `surveys/`) | Supabase Storage (`reservations/`, `surveys/`) | Supabase Storage (`reservations/`, `surveys/`) |
| 認証 | スキップ | Supabase Auth | Supabase Auth | Supabase Auth |
| RLS | 無効 | 有効 | 有効 | 有効 |
| CI/CD | - | Vercel プレビュー | GitHub Actions + Vercel | GitHub Actions + Vercel |
| プロジェクト分離 | - | staging の dev ブランチを使用 | 既存プロジェクト継続使用 | 新規プロジェクト作成 |
