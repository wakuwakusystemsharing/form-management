...
11. [本プロジェクト固有の設定](#11-本プロジェクト固有の設定)
...
NEXT_PUBLIC_SUPABASE_ANON_KEY=sb_publishable_xxx  # 新形式キー
```

**高権限キー (Secret) の保護**
```bash
# バックエンド環境変数 (.env にのみ、git ignore)
SUPABASE_SECRET_KEY=sb_secret_xxx  # 新形式キー
```

**本プロジェクトでの環境変数名**

本プロジェクトでは従来の JWT 形式のキー名（後方互換）を使用しています:

```env
# フロントエンド（公開可能 / publishable 相当）
NEXT_PUBLIC_SUPABASE_URL=https://your-project.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...

# バックエンドのみ（秘密鍵 / secret 相当）
SUPABASE_SERVICE_ROLE_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
```

キーの取得先: Supabase Dashboard → Project Settings → API → Legacy Keys
...
初回セットアップ時に SQL Editor で実行するポリシー設定です:
...
-- 2. サービス管理者用ポリシー（全データアクセス可能、Admin Client が RLS をバイパス）
-- NOTE: API ルートはサービス管理者のメール確認後、Admin Client (service_role) で直接アクセス
-- 以下は参考用（アプリケーションコードは Admin Client を優先使用）
...
- **サービス管理者**: ハードコードされたメールで識別 → `getSupabaseAdminClient()` (service_role) で RLS バイパス
- **店舗管理者**: Supabase Auth → `store_admins` テーブルで `store_id` を紐付け → RLS 適用
- **Local 環境**: `shouldSkipAuth()` = true → すべての認証をスキップ

```typescript
// src/lib/supabase.ts より
// サービス管理者向け（RLS バイパス）
const adminClient = getSupabaseAdminClient();  // service_role キーを使用

// 店舗管理者向け（RLS 適用）
const userClient = getSupabaseClient();  // anon キーを使用
```

### 管理者アカウント作成

初回セットアップ時にサービス管理者アカウントを作成する場合:
...
`supabase/migrations/` 内のファイルを SQL Editor で順番に実行します。詳細は [`DATABASE_MIGRATION.md`](DATABASE_MIGRATION.md) を参照してください。
