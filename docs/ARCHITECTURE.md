...
```
Web 予約フォーム（form_type === 'web'）から予約送信
  ↓
/api/reservations の POST 内で fire-and-forget 呼び出し
  ↓
src/lib/reservation-email.ts: sendReservationEmails()
  ├─ src/lib/email-templates.ts: buildCustomerConfirmationEmail() → お客様向け
  └─ src/lib/email-templates.ts: buildStoreNotificationEmail()    → 店舗向け
  ↓
src/lib/email-sender.ts: sendEmail() — Resend ラッパ
  ├─ EMAIL_FROM_ADDRESS のメアド部分は固定（Resend で DNS 認証済み）
  ├─ 表示名は店舗名で動的差し替え（例: 'モテストシナモロール <noreply@send.example.com>'）
  └─ Reply-To に store.owner_email（顧客の返信が店舗オーナーに直接届く）
  ↓
Resend API → 受信者の受信トレイ
```

- **送信失敗時**: 予約自体は成功扱い、ログのみ
- **API キー未設定時**: ログ出力のみでスキップ（local 開発で安全）
- **設定**: `RESEND_API_KEY` / `EMAIL_FROM_ADDRESS` 環境変数

## 🗓️ 祝日判定ロジック（予約フォーム内）

`src/lib/static-generator-reservation.ts` がフォーム HTML 内に約 80 行の JS を埋め込み、ランタイムで日本の祝日を計算する:

- 1980-2099 年対応（春分・秋分の日は天文学的近似式）
- 振替休日と国民の休日を自動判定
- 設定 `calendar_settings.holidays_as_closed` ON で祝日のセルを ✕ 表示
- `excluded_holiday_types` で「振替休日だけ営業可」のような個別除外も可能
- 外部 API 呼び出しなしで完結（顧客側のロード時間に影響なし）
...
interface Store {
  ...
  name: string;
  owner_name: string;
  owner_email: string;        // メール通知の Reply-To / 店舗側通知のフォールバック宛先
  phone?: string;             // メール本文に差し込み
  postal_code?: string;       // 郵便番号、メール本文に「〒xxx-xxxx」で差し込み
  address?: string;           // メール本文に差し込み
  description?: string;
  website_url?: string;
  ...
  google_calendar_refresh_token?: string;             // store_oauth 時の暗号化リフレッシュトークン
...
```
顧客が静的フォーム（Supabase Storage）を開く
  ↓ name / phone / [email] / メニュー / 日時 / カスタムフィールドを入力
  ↓ 「予約する」ボタン
POST /api/reservations
  ...
  └─ 6. Web 予約のときメール送信（form_type === 'web'、fire-and-forget）
        ...
        └─ 店舗向け:   notification_email > owner_email へ
  ↓
レスポンス: 201 + reservation オブジェクト
  ↓
LIFF フォームの場合:
  └─ liff.sendMessages([本文, (任意で 2 通目テキスト)])
     → 公式 LINE webhook が反応 → Flex メッセージで予約確認返信
```

### 7. LINE リマインダー送信フロー

```
pg_cron が毎時実行（hourly）
  ↓
Supabase Edge Function: send-reminders
  ├─ 1. 翌日（JST）が予約日の reservations を取得
